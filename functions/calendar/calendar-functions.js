...
const testCalendarId =
  ...
  "3313765c4e9242a84e058797b11437b1ab1034a05ba19e1d4885aa9cf0838195@group.calendar.google.com";
...
async function maybeRefundPoints(event) {
  ...
  if ("recurrence" in event) {
    ...
    studentsQuery.docs
      .filter((doc) => studentIdList.includes(doc.id))
...
exports.get_event = functions
  ...
  .https.onCall((data, context) => {
    ...
    return calendarUtils.getEvent(query).catch((err) => {
      console.error(err);
      throw new functions.https.HttpsError("internal", err);
    });
  });
...
exports.list_events = functions
  ...
  .https.onCall((data, context) => {
    ...
    return calendarUtils.listEvents(query).catch((err) => {
      console.error(err);
      throw new functions.https.HttpsError("internal", err);
    });
  });
...
exports.list_instances = functions
  ...
  .https.onCall((data, context) => {
    ...
    return calendarUtils.listInstances(query).catch((err) => {
      console.error(err);
      throw new functions.https.HttpsError("internal", err);
    });
  });
  ...
  .https.onCall((data, context) => {
    ...
    return Promise.all([
      calendarUtils
        ...
        .then((event) => maybeRefundPoints(event)),
